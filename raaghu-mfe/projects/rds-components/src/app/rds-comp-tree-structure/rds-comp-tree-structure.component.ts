...
import { TreeType } from '../../models/pemission.model';
import { TreeStructureNode } from '../../models/tree-structure.model';

@Component({
  selector: 'rds-comp-tree-structure',
  templateUrl: './rds-comp-tree-structure.component.html',
  styleUrls: ['./rds-comp-tree-structure.component.scss']
})
export class RdsCompTreeStructureComponent implements OnInit {
  @Input() isReset: boolean = false;
  @Input() isEdit: boolean = false;
  @Input() selectedItems: TreeStructureNode[] = [];
  @Input() selectAllLabel: string = 'Select All';
  @Input() treeData: TreeStructureNode[];
  ...
  selectedNodeList: TreeStructureNode[] | null;
  selectedNestedNode: TreeStructureNode | null;
  SelectAll: boolean = false;
  nodeLabel!: string;
  itemCode: any;
  ItemDescription: any;
  addNode(nodeArray: TreeStructureNode[]) {
    this.itemCode = '';
    this.ItemDescription = '';
    this.selectedNodeList = nodeArray;
  }
  addNestedNode(node: TreeStructureNode) {
  ...
  setStateBasedOnMutable(lastNode: boolean, node: TreeStructureNode) {
  ...
  selectNode(node: any, event: any) {
    if (this.multiselectTree) {
      ...
      const selectedNodes: TreeStructureNode[] = this.getSelectedNodes(this.treeData);
  ...
  getSelectedNodes(treeData: any): TreeStructureNode[] {
  ...
  checkForParent(treeData: TreeStructureNode[], node: any, event: boolean): void {
    treeData && treeData.forEach((tree: any) => {
  ...
  selectAllNode(node: TreeStructureNode, checked: boolean) {
  ...
  checkNodSelectOrnot(node: TreeStructureNode): boolean {
